...
set.seed(10)
# pscore model
beta.mod1 <- c(-3, .33*colMeans(d.mod1[,-(1:2)])/ apply(d.mod1[,-(1:2)], 2, sd)) # d.mod1[,-(1:2)] 表示去除前两列，即Y，A
g1W.mod1 <- plogis(cbind(1, as.matrix(d.mod1[,-(1:2)])) %*% beta.mod1) # plogis(x) == (1+ tanh(x/2))/2, and it is called a sigmoid function in contexts such as neural network

# A 抽样
d.mod1$A <- rbinom(n, 1, g1W.mod1) #eg. Find 8 random values from a sample of 150 with probability of 0.4. ， x <- rbinom(8,150,.4)
# Use glm model as truth for this simulation
beta.Q.mod1 <- -coef(m.or)
beta.Q.mod1[1] <- .25
beta.Q.mod1[2] <- 1  # coef on A
logit.drs.mod1 <-  cbind(1, as.matrix(d[,-(1:2)])) %*% beta.Q.mod1[-2] # %*% 表示矩阵相乘；  beta.Q.mod1[-2] 表示去除第二个元素
# Y 抽样
...
n.b <- 500
for (i in mod1_files){
	b <- sample(1:n, n.b, replace = TRUE) # 有放回的抽样
	d.mod1$A[b] <- rbinom(n.b, 1, g1W.mod1[b])# A 抽样生成
	d.mod1$Y[b] <- rbinom(n.b, 1, plogis(d.mod1$A[b] + logit.drs.mod1[b] ))# Y 抽样生成
...
g1W.mod2 <- plogis(cbind(1, d[,15],W.interaction, W.ratio) %*% beta.mod2)# 给特征乘上一个随机权重
# A 抽样
  A.mod2 <- rbinom(n, 1, g1W.mod2)
 beta.Q.mod2 <- c(-1.8, runif(ncol(W.interaction) + ncol(W.ratio), -1, 1)) 
logit.drs.mod2 <-  cbind(1, W.interaction, W.ratio) %*% beta.Q.mod2 # 给特征乘上一个随机权重
# Y 抽样
